...
def attribute_info(selected_attr_label: str) -> str:
    ...
    """
    # Friendly names mapped to your uploaded files
    ...
    if selected_attr_label == "Percentage Impervious Cover":
        return (
            "**Why it matters.** Impervious surfaces (streets, sidewalks, roofs) limit infiltration, raise runoff "
            "coefficients, and increase peak discharge (e.g., Rational Method). Higher imperviousness generally "
            "elevates pluvial flood susceptibility. — *" + P2 + "*"
        )

    if selected_attr_label == "Number of Buildings":
        return (
            "**Why it matters.** Buildings add large, contiguous impervious area and obstruct overland flow. "
            "In NYC analyses, building variables (counts/footprint) rival or exceed elevation/percent impervious in "
            "explaining spatial variability of reported flooding. — *" + P1 + "*"
        )

    if selected_attr_label == "Building Footprint":
        return (
            "**Why it matters.** Greater footprint concentrates impervious cover and narrows flow paths, amplifying "
            "runoff and ponding; building metrics were among strong predictors in NYC. — *" + P1 + "*"
        )
    ...
    if selected_attr_label == "Commuting":
        return (
            "**Why it matters (and a caution).** ‘Drive-alone’ commuting correlated with higher reported street flooding "
            "in NYC—capturing exposure (drivers encounter ponded roads) and potential reporting bias in crowdsourced 311 "
            "data. — *" + P1 + "*"
        )
