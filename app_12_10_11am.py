...
# =====================================================================
# Excel-backed attributes (NRI + statics)  **NEW**
# =====================================================================
...
def _normalize_geoids_from_parts(state, county, tract) -> str:
    # STATE (2), COUNTY (3), TRACT (6) => 11 total
...
def _normalize_geoid_any(s: str) -> str:
    d = _digits_only(s)
    if len(d) >= 11:
        # Take the **last** 11 in case inputs had state+county+tract+block etc.
        return d[-11:]
...
def load_risk_xlsx(path_or_bytes) -> pd.DataFrame:
    ...
    if all(c in df.columns for c in ["STATEFIPS", "COUNTYFIPS", "TRACT"]):
        ...
        # Try common single-column ids in order of likelihood
        ...
        if candidates:
            ...
            # Fallback: attempt to detect any column that looks like ID
            guess = None
            for c in df.columns:
...
@st.cache_data
def merge_attrs_into_tracts(_boards: list, df: pd.DataFrame) -> tuple[list, float]:
    """
    Cache-safe merge (leading underscore fixes UnhashableParamError).
    Joins spreadsheet attributes into each tract's attrs by robust GEOID normalization.
    Returns (boards_with_attrs, coverage_pct).
    """
    lut = df.set_index("__join_key__").to_dict("index")
    matched = 0
    out = []

    # Common feature keys we’ll try on the shapefile/geojson side
    feature_keys = ["GEOID","GEOID10","GEOID20","TRACTFIPS","TRACTCE","TRACT","NRI_ID","tract_id","unit_id"]

    for b in _boards:
        attrs = dict(b.get("attrs", {}) or {})
        feature_geoid = ""
        # Build from parts if available in feature attrs
        if all(k in attrs for k in ["STATEFIPS","COUNTYFIPS","TRACT"]):
            feature_geoid = _normalize_geoids_from_parts(attrs.get("STATEFIPS"), attrs.get("COUNTYFIPS"), attrs.get("TRACT"))
        else:
            # Otherwise try a list of single columns
            tried = []
            for k in feature_keys:
                v = attrs.get(k) if k in attrs else b.get(k)
                if v not in (None, ""):
                    tried.append(k)
                    g = _normalize_geoid_any(str(v))
                    if g:
                        feature_geoid = g
                        break
            # As a last resort, try the display id derived earlier
            if not feature_geoid:
                feature_geoid = _normalize_geoid_any(get_board_display_id(b))

        # Attempt the lookup
        merged = lut.get(feature_geoid)
        if merged:
            attrs.update(merged)
            matched += 1

        bb = dict(b); bb["attrs"] = attrs
        out.append(bb)

    return out, matched / max(1, len(_boards))
...
# --------------------
# Robust quartile binning with safe fallbacks  **REPLACE THIS FUNCTION**
# --------------------
def quantile_bins_4(series: pd.Series) -> tuple[pd.Series, np.ndarray]:
    """
    Returns (bin[0..3], edges[5]) where edges are raw-value edges used for the legend.
    - If all values are equal (or nearly so), falls back to linspace edges.
    - Guarantees len(labels) == len(edges)-1 to satisfy pandas.cut.
    """
    ...
    # If all (or almost all) values are identical → even edges
    if np.isclose(vmin, vmax):
        edges = np.linspace(vmin, vmax + 1e-9, 5)
    else:
        edges = np.quantile(s, [0, 0.25, 0.5, 0.75, 1.0]).astype(float)
        # Ensure strictly non-decreasing and expand if needed
        edges = np.maximum.accumulate(edges)
        # If quantiles collapsed (e.g., many identical values), force even edges
        unique_edges = np.unique(edges)
        if unique_edges.size < 5:
            edges = np.linspace(vmin, vmax, 5)

        # Guard again in case vmin==vmax after cleaning
    ...
    # If pandas still returns NA (pathological tiny ranges), fill with 0
...
# =====================================================================
# Methodology text (expanded for Claude)
# =====================================================================
...
def ensure_boards() -> list:
    ...
    if os.path.exists(base_path + '.shp'):
        ...
        if boards:
            # --- NEW: merge the spreadsheet into the board attrs ---
            try:
                df_excel = load_risk_xlsx(RISK_XLSX)  # uses /mnt/data/Risk_Attributes_Table_v4.xlsx by default
                boards, cov = merge_attrs_into_tracts(boards, df_excel)
                st.caption(f"Excel merge coverage: {cov:.1%}")
            except Exception as e:
    ...
    if boards:
        # --- NEW: merge spreadsheet here too (when uploading geometry) ---
        try:
...
def urban_features_page():
    ...
    # >>> NEW: take values from the spreadsheet (merged) <<<
    boards_raw = boards
    if os.path.exists(RISK_XLSX):
        df_x = load_risk_xlsx(RISK_XLSX)
        boards, coverage = merge_attrs_into_tracts(boards_raw, df_x)
        if coverage < 0.75:
            st.warning(f"Excel merge coverage: {coverage*100:.1f}% — check tract IDs.", icon="⚠️")
    else:
        st.info("Risk_Attributes_Table_v4.xlsx not found; using attributes already on the layer.")

    vals = {b["unit_id"]: get_attr_num(b.get("attrs",{}), sel_attr) for b in boards}
    series = pd.Series(vals).sort_index()

    # Build bins & colors from RAW values (quartiles) and render legend with ranges
    ...
    deck = pydeck_choropleth(fc, lat_c, lon_c, zoom_c, prop_name="value",
                             tooltip_label=sel_label, opacity=0.90, label_layer=label_layer)
...
# ------------------------------------------------------------
# REPLACED: Risk Mapping page with spreadsheet-backed values
# ------------------------------------------------------------
def page_risk_mapping():
    ...
    # Allow fallback if the exact field name isn't present
    ...
    for b in boards:
        ...
        num = coerce_nonneg_numeric(v_raw, 0.0)  # blanks → 0
        raw_vals[b["unit_id"]] = num
    raw_series = pd.Series(raw_vals).sort_index()

    # -----------------------------
    # Bin by quartiles of RAW values (and derive nice numeric ranges)
    # quantile_bins_4 returns (bins, edges) where bins ∈ {0,1,2,3} and len(edges)=5
    # -----------------------------
    ...
    fc = fc_from_boards_and_values(
        boards,
        raw_series,                 # keep raw values so tooltip shows the real number
        prop_name="risk_value",
        color_series=color_series   # coloring driven by quartiles
    )
    ...
    with c2:
        # For PNG, pass bin indices as floats so PALETTE4 is used consistently
...
def page_uhi():
    ...
    # Merge spreadsheet once
    if os.path.exists(RISK_XLSX):
        df_x = load_risk_xlsx(RISK_XLSX)
        boards, coverage = merge_attrs_into_tracts(boards, df_x)
        if coverage < 0.75:
            st.warning(f"Excel merge coverage: {coverage*100:.1f}% — check tract IDs.", icon="⚠️")
    else:
        st.error("Risk_Attributes_Table_v4.xlsx not found."); st.stop()

    field_name, pretty = "HWAV_RISKS", "NRI Heat-Wave Risk"
    series = pd.Series({b["unit_id"]: get_attr_num(b.get("attrs", {}), field_name) for b in boards}).sort_index()

    bins, edges = quantile_bins_4(series)
    color_map = {0: PALETTE4[0], 1: PALETTE4[1], 2: PALETTE4[2], 3: PALETTE4[3]}
    color_series = pd.Series({uid: color_map[bins.loc[uid]] for uid in series.index})

    fc = fc_from_boards_and_values(boards, series, prop_name="uhi_raw", color_series=color_series)

    lat_c, lon_c, zoom_c = boards_bbox({"type": "FeatureCollection",
                                        "features": [{"type": "Feature", "geometry": b["feature_geom"]} for b in boards]})
    label_layer = cb_label_layer_from_boards(boards, text_size=10)
    deck = pydeck_choropleth(fc, lat_c, lon_c, zoom_c, prop_name="uhi_raw",
                             tooltip_label=f"{pretty} (raw value)", opacity=0.90, label_layer=label_layer)
    c_map, c_leg = st.columns([4,1])
    with c_map: st.pydeck_chart(deck, use_container_width=True)
    with c_leg: st.markdown(legend_with_ranges_html(pretty, edges, colors=PALETTE4), unsafe_allow_html=True)
