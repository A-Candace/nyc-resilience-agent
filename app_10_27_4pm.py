...
# =========================
def build_board_multiplier(boards, weight_set_key="Average (Model 1 & 2)"):
...
def sidebar_agents():
    with st.sidebar:
        ...
        st.checkbox("Forecasting Agent", value=True, disabled=True)  # show enabled in stack
...
This exact series is used here; we **do not** regenerate precipitation.
    ...
    # 1) Choose weight set & model
    st.subheader("1) Static-variable weighting and model")
    col1, col2 = st.columns(2)
    with col1:
        weight_key = st.selectbox("Static importance set", list(WEIGHT_SETS.keys()), index=2)
    with col2:
        model = st.selectbox(
            "Forecasting model",
            ["Rule-based (multiplier × precip)",
