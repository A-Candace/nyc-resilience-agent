...
# Web scraping for Community Board Agent
import re
...
from forecasting.static_modifiers import compute_static_matrix, robust_scale, static_risk_weight_per_board, to_multiplier
...
def build_board_multiplier(boards, weight_set_key="Random Forest (Model 1)"):
    """
    Build per-board multipliers using ONLY Model 1 weights (as requested).
    """
    # Force to Model 1 even if some other default creeps in
    if "Random Forest (Model 1)" in WEIGHT_SETS:
        weight_set_key = "Random Forest (Model 1)"
    weights = WEIGHT_SETS[weight_set_key]  # dict of six normalized weights
...
def normalize_cb_id(val):
    ...
    Falls back gracefully if not numeric.
    """
...
def load_and_merge_board_data(boards: list, csv_path: str = "DataForBoxPlots.csv") -> list:
    ...
    try:
        ...
        # Normalize CB_id to string digits
        ...
        for board in boards:
            ...
            filtered_attrs = {
                'boro_cd': boro_cd,  # Use normalized version (clean digits)
...
SESSION_KEYS = {
    ...
    "last_clicked_cb": "last_clicked_cb"       # stash a CB code for CB Agent
}
...
# -----------------------------
# Community Board Agent (Databook scraper)
# -----------------------------
DATABOOK_BASE = "https://databook.wegov.nyc"

def _cb_databook_urls(cb_code: str) -> Dict[str, str]:
    cb_code = normalize_cb_id(cb_code)
    return {
        "district": f"{DATABOOK_BASE}/d/{cb_code}",
        "census_fallback": f"https://popfactfinder.planning.nyc.gov/profile/geo/{cb_code}"
    }

def _clean_text(s: str) -> str:
    return re.sub(r"\s+", " ", s).strip()

def fetch_cb_directory(cb_code: str) -> Dict[str, Optional[str]]:
    """
    Scrape the district page for the Directory card:
    Chair, District Manager, address, phones, email, website, meeting times, census link.
    Returns a dict; values may be None if not found or scraping disabled.
    """
    out = {
        "cb_code": normalize_cb_id(cb_code),
        "chair": None,
        ...
        "website": None,
        "census_link": None,
        "databook_url": _cb_databook_urls(cb_code)["district"]
    }
    if not SCRAPE_OK:
        out["note"] = "requests/bs4 not installed; enable to fetch directory."
        return out

    try:
        urls = _cb_databook_urls(cb_code)
        r = requests.get(urls["district"], timeout=10)
        if r.status_code != 200:
            out["note"] = f"HTTP {r.status_code} fetching district page."
            return out
        soup = BeautifulSoup(r.text, "html.parser")

        # Try to locate the directory card by scanning for common labels
        text = soup.get_text(" ", strip=True)

        # Heuristics for each field:
        patterns = {
            "chair": r"Chair\s*([^\n]+)",
            "district_manager": r"District Manager\s*([^\n]+)",
            "office_address": r"Office Address\s*([^\n]+)",
            "office_phone": r"Office Phone\s*([^\n]+)",
            "office_fax": r"Office Fax\s*([^\n]+)",
            "office_email": r"(?:Office Email|Email)\s*([^\s]+@[^\s]+)",
            "board_meeting": r"Board Meeting\s*([^\n]+)",
            "cabinet_meeting": r"Cabinet Meeting\s*([^\n]+)",
        ...
        for key, pat in patterns.items():
            ...
            if m:
                out[key] = _clean_text(m.group(1))

        # Try to find explicit census profile link if present in anchors
        census_link = None
        for a in soup.find_all("a"):
            if "census" in (a.get_text("") or "").lower() or "profile" in (a.get_text("") or "").lower():
                href = a.get("href")
                if href and ("popfactfinder" in href or "/profile" in href):
                    census_link = href if href.startswith("http") else f"{DATABOOK_BASE}{href}"
                    break
        out["census_link"] = census_link or _cb_databook_urls(cb_code)["census_fallback"]

        return out
    except Exception as e:
        out["note"] = f"Error parsing district page: {e}"
        return out


def render_cb_agent_ui(source_label: str, default_cb: Optional[str] = None):
    """
    Renders a small UI to fetch & display Community Board directory info.
    `source_label` indicates where it's used ("Sidebar" or "Main").
    """
    st.markdown(f"**Community Board Agent (beta)** — {source_label}")
    cb_code = st.text_input("Enter Community Board code (e.g., 308):", value=default_cb or "", key=f"cb_code_{source_label}")
    colx1, colx2, colx3 = st.columns([1,1,2])
    with colx1:
        go = st.button("Fetch", key=f"fetch_{source_label}")
    with colx2:
        # Quick open buttons enabled after fetch or if code present
        open_databook = st.button("Open Databook page", key=f"open_db_{source_label}")
    with colx3:
        open_census = st.button("Open Census profile", key=f"open_cen_{source_label}")

    if cb_code and open_databook:
        urls = _cb_databook_urls(cb_code)
        st.markdown(f"[Open Databook for CB {normalize_cb_id(cb_code)}]({urls['district']})")
    if cb_code and open_census:
        urls = _cb_databook_urls(cb_code)
        st.markdown(f"[Open Census profile for CB {normalize_cb_id(cb_code)}]({urls['census_fallback']})")

    if go and cb_code:
        info = fetch_cb_directory(cb_code)
        if info.get("databook_url"):
            st.markdown(f"[Databook page]({info['databook_url']})  |  "
                        f"[Census profile]({info.get('census_link') or _cb_databook_urls(cb_code)['census_fallback']})")
        cols = st.columns(2)
        with cols[0]:
            st.write(f"**Chair:** {info.get('chair') or '—'}")
            st.write(f"**District Manager:** {info.get('district_manager') or '—'}")
            st.write(f"**Board Meeting:** {info.get('board_meeting') or '—'}")
            st.write(f"**Cabinet Meeting:** {info.get('cabinet_meeting') or '—'}")
        with cols[1]:
            st.write(f"**Office Address:** {info.get('office_address') or '—'}")
            st.write(f"**Office Phone:** {info.get('office_phone') or '—'}")
            st.write(f"**Office Fax:** {info.get('office_fax') or '—'}")
            st.write(f"**Office Email:** {info.get('office_email') or '—'}")
...
def render_map_assistant(block_key: str, context: dict, default_hint: str, describe_by_cb: bool = False):
    ...
    If describe_by_cb=True, Claude is explicitly prompted to describe **by community board number**.
    """
    ...
    with cols[0]:
        if st.button("Describe this map", key=f"describe_{block_key}"):
            ...
            extra = "\n\nIMPORTANT: Refer to locations by their community board NUMBER only (e.g., 305), not by names."
            prompt = (
...
def sidebar_agents():
    with st.sidebar:
        ...
        st.divider()
        # Community Board Agent (Sidebar expander)
        with st.expander("Community Board Agent (beta)", expanded=False):
            if not SCRAPE_OK:
                st.warning("Install `requests` and `beautifulsoup4` to enable scraping.")
            render_cb_agent_ui("Sidebar")
...
def landing_page():
    ...
    st.graphviz_chart('''
        digraph G {
            ...
            "Forecasting Agent" -> "Community Board Agent (beta)" [label="explain hotspots, contacts, budgets*"];
            "Community Board Agent (beta)" -> "Optimization Agent" [style=dashed, label="(soon)"];
...
def flooding_mapping_agent():
    ...
    st.caption("Displays Community Board polygons, places one radar point per board, simulates precipitation, and colors the boards (graduated blues). Saves the forecast for downstream agents.")
    ...
    # add clean cb column for clarity
    ...
    # convert to CB numbers for context
    ...
    if not has_merged_data:
        ...
        if attr_series.max() == 0 and attr_series.min() == 0:
            ...
            render_map_assistant(
                block_key=f"attr_{selected_attr}",
    ...
    with st.expander("Date range (inclusive end date) and generation", expanded=True):
        ...
        # Let the user choose baseline aggregation over radars
    ...
    if start_date >= end_date:
        ...
        if generate:
            with st.spinner("Generating hourly precipitation and computing daily metrics..."):
                # NOTE: Use datetime.min.time() (not datetime.min_time) to avoid AttributeError
                ...
                # Save daily tables to session for convenience
                ...
                # >>>>>>> SHARED PRECIP FOR DOWNSTREAM AGENTS (baseline) <<<<<<<
                # Build a single citywide baseline time series using chosen aggregation
    ...
    # ---- Community Board Agent (Main) ----
    st.divider()
    with st.expander("Community Board Agent (beta) – Learn more about a board", expanded=False):
        # seed with last clicked if ever stored
        last_cb = st.session_state.get(SESSION_KEYS["last_clicked_cb"], "")
        render_cb_agent_ui("Main", default_cb=last_cb)
...
def forecasting_agent():
    ...
    # Require that the Mapping agent ran
    ...
    precip_meta = st.session_state.get(SESSION_KEYS["precip_meta"])
    ...
    with st.expander("See agent handoff", expanded=True):
        st.graphviz_chart('''
            digraph G {
                ...
                "Forecasting Agent" -> "Community Board Agent (beta)" [label="context & stakeholders"];
                "Forecasting Agent" -> "User" [label="Explain + QA"];
            }
        ''')
        st.write("**Received precipitation (read-only, shared):**")
        st.dataframe(precip_df.head(12), use_container_width=True, hide_index=True)
        if precip_meta:
            st.caption(f"Source: {precip_meta.get('source')} | "
                       f"Start: {precip_meta.get('start')} | End (exclusive): {precip_meta.get('end_exclusive')} | "
                       f"Aggregation: {precip_meta.get('aggregation')} | Radar points: {precip_meta.get('radar_points')} | "
                       f"Baseline method: {precip_meta.get('baseline_method')}")
...
- Static multiplier: Derived from scaled **Buildings, Footprint, Impervious, Slope, Elevation, Commuting** via the Model 1 weight set.
...
They are robustly scaled to ~[0,1] and combined with the **Model 1** weight set to form a **static multiplier** that up/down-weights the precipitation signal per board.
            """)
    with col_info2:
...
This exact series is used here; we **do not** regenerate precipitation.
    ...
    # 1) Choose weight set & model (weights menu restricted to Model 1)
    st.subheader("1) Static-variable weighting and model")
    col1, col2 = st.columns(2)
    with col1:
        model1_key = "Random Forest (Model 1)" if "Random Forest (Model 1)" in WEIGHT_SETS else list(WEIGHT_SETS.keys())[0]
        weight_key = st.selectbox("Static importance set", [model1_key], index=0)
    with col2:
        model = st.selectbox(
            "Forecasting model",
            ["Rule-based (multiplier × precip)",
    ...
    # Build per-board multiplier once (also expose static table for transparency)
    ...
    if model.startswith("Rule-based"):
        ...
        # Need board centroids for graph
        cent = pd.DataFrame({
            "unit_id": [b["unit_id"] for b in boards],
    ...
    deck = pdk.Deck(
        layers=[esri_light_gray_basemap(), layer],
        initial_view_state=pdk.ViewState(latitude=lat_c, longitude=lon_c, zoom=zoom_c),
    ...
    # --- Per-result assistant: Forecast map QA (by CB number) ---
    context_forecast = {
    ...
    # 4) New: Select a time range and map the **mean flood risk** per board over that range
    st.divider()
    st.subheader("4) Aggregate forecast over a time range (mean flood index per board)")
    # Ensure ts are sorted
    ...
    deck2 = pdk.Deck(
        layers=[esri_light_gray_basemap(), layer2],
        initial_view_state=pdk.ViewState(latitude=lat_c, longitude=lon_c, zoom=zoom_c),
    ...
    # Claude QA for the aggregated map
    context_range = {
    ...
    # Hook / Reminder for Community Board Agent path from Forecasting
    st.info("🔎 Want context on stakeholders & logistics for a hotspot? Open **Community Board Agent (beta)** in the sidebar.")
