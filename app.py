...
def sidebar():
    with st.sidebar:
        ...
        if st.button("📈 Forecasting", use_container_width=True, key="nav_forecast_construction", disabled=True):
            pass
        st.caption("Coming soon: Next-day flood predictions")
        
        if st.button("👥 Demographics", use_container_width=True, key="nav_demographics_construction", disabled=True):
            pass
        st.caption("Coming soon: Population & vulnerability data")
        
        if st.button("🌦️ Historical Weather", use_container_width=True, key="nav_weather_construction", disabled=True):
            pass
