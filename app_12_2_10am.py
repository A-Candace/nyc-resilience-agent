...
MLFLOW_EXPERIMENT = os.getenv("MLFLOW_EXPERIMENT", "hackathon-agent")
if MLFLOW_TRACKING_URI:
    mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
mlflow.set_experiment(MLFLOW_EXPERIMENT)

# ---- SAFE MLflow span helper (works even if tracing/SpanType not present) ----
def mlflow_span(name: str):
    """
    Returns a context manager for an MLflow span if supported, otherwise a no-op.
    Tries to set span_type=INTERNAL when available, but won't fail if enum is missing.
    """
    try:
        # MLflow >= 2.14 (tracing moved), but keep broad try for compatibility
        from mlflow.tracing import start_span as _start_span  # type: ignore
        try:
            from mlflow.entities import SpanType as _SpanType  # type: ignore
            # If INTERNAL exists, use it; else let start_span pick defaults
            internal = getattr(_SpanType, "INTERNAL", None)
            if internal is not None:
                return _start_span(name=name, span_type=internal)
            return _start_span(name=name)
        except Exception:
            # No SpanType in this build
            return _start_span(name=name)
    except Exception:
        # Tracing not available in this MLflow build
        return nullcontext()
...
SESSION_KEYS = {
    ...
    "lon": "lon_c",
    "zoom": "zoom_c",
    "precip_df": "precip_forecast_df",         # required by Forecasting agent (citywide baseline)
    ...
    # NEW: per-board per-hour precipitation (spatial) for Forecasting Agent toggle
    "precip_by_board_long": "precip_by_board_long",
}

# === Shared timestamp key helper (for cross-agent joins) ===
def _ts_key(ts):
    ts = pd.Timestamp(ts)
    if ts.tz is None:
...
def sidebar_agents():
    with st.sidebar:
        ...
        if bedrock_enabled():
            ...
            st.info("Claude: missing " + ", ".join(missing) + " — set via env or (on Streamlit Cloud) in Secrets.")
...
def flooding_mapping_agent():
    ...
    with st.expander("Date range (inclusive end date) and generation", expanded=True):
        ...
        # Use two date pickers (avoid tuple-return issue)
        start_date = st.date_input(
            "Start date",
            value=st.session_state.get("range_start_date", default_start),
        ...
        end_date = st.date_input(
            "End date (inclusive)",
            value=st.session_state.get("range_end_date", default_end),
            min_value=date(2025, 10, 6),
            max_value=date(2030, 10, 6),
            key="range_end_date",
        )
    ...
    if generate:
        with st.spinner("Generating hourly precipitation and computing daily metrics..."):
            ...
            # NEW: compute per-board, per-hour precipitation and cache for Forecasting Agent
            # Build a DataFrame with columns: timestamp(str), unit_id, precip_mm_hr_board
            cell_ids = RADAR_DF["cell_id"].tolist()
            hours_str = [pd.Timestamp(ts).isoformat() for ts in hours_idx]
            rows = []
            # Prepare lookup of points for nearest fallback
            pt_lookup = {r["cell_id"]: (r["lat"], r["lon"]) for _, r in RADAR_DF.iterrows()}
            sg, _, _, _ = try_import_shapely()

            # Precompute point objects for speed
            point_objs = {cid: sg.Point(pt_lookup[cid][1], pt_lookup[cid][0]) for cid in cell_ids}

            # Precompute cells inside each board + nearest fallback
            board_cells = {}
            board_nearest = {}
            for b in boards:
                geom = b["geom"]
                inside = [cid for cid in cell_ids if geom.contains(point_objs[cid])]
                board_cells[b["unit_id"]] = inside
                if inside:
                    board_nearest[b["unit_id"]] = inside[0]
                else:
                    # nearest single radar by distance
                    c = geom.representative_point()
                    latc, lonc = float(c.y), float(c.x)
                    best_cid, bestd = None, 1e18
                    for cid in cell_ids:
                        plat, plon = pt_lookup[cid]
                        d = haversine_km(latc, lonc, plat, plon)
                        if d < bestd:
                            bestd = d
                            best_cid = cid
                    board_nearest[b["unit_id"]] = best_cid

            # Loop hours and aggregate quickly using precomputed membership
            for h_idx, ts in enumerate(hours_str):
                row_vals = precip_matrix[h_idx, :]  # same order as cell_ids
                val_by_cid = {cid: float(row_vals[i]) for i, cid in enumerate(cell_ids)}
                for b in boards:
                    uid = b["unit_id"]
                    inside = board_cells.get(uid, [])
                    if inside:
                        v = float(np.mean([val_by_cid[cid] for cid in inside]))
                    else:
                        v = float(val_by_cid.get(board_nearest[uid], 0.0))
                    rows.append({"timestamp": ts, "unit_id": uid, "precip_mm_hr_board": v})

            precip_by_board_long = pd.DataFrame(rows)
            # ---- ADD ts_key column for robust joins ----
            precip_by_board_long["ts_key"] = precip_by_board_long["timestamp"].apply(_ts_key)
            st.session_state[SESSION_KEYS["precip_by_board_long"]] = precip_by_board_long
            ...
            # ---- ADD ts_key column here too ----
            precip_df["ts_key"] = precip_df["timestamp"].apply(_ts_key)
            ...
            # ---- MLflow logging: data generation run ----
            import tempfile, os  # avoid re-importing mlflow shadowing

            with mlflow.start_run(run_name=f"range_{start_date}_to_{end_date}"):
                # Params – how we generated data
                mlflow.log_params({
                    "range_start": start_date.isoformat(),
                    "range_end_inclusive": end_date.isoformat(),
                    "baseline_method": st.session_state.get("baseline_method_choice", "Mean over radars"),
                    "radar_point_count": int(len(RADAR_DF)),
                    "board_count": int(len(boards))
                })

                # Quick summary metrics
                try:
                    mlflow.log_metric("precip_citywide_mean", float(pd.to_numeric(precip_df["precip_mm_hr"], errors="coerce").mean()))
                    mlflow.log_metric("precip_citywide_max", float(pd.to_numeric(precip_df["precip_mm_hr"], errors="coerce").max()))
                except Exception:
                    pass

                # Artifacts – baseline, per-board hourly, and daily tables if present
                with tempfile.TemporaryDirectory() as td:
                    precip_df.to_csv(os.path.join(td, "precip_baseline.csv"), index=False)
                    st.session_state[SESSION_KEYS["precip_by_board_long"]].to_csv(
                        os.path.join(td, "precip_by_board_hourly.csv"), index=False
                    )
                    if "daily_max_long" in st.session_state:
                        st.session_state["daily_max_long"].to_csv(os.path.join(td, "daily_max_long.csv"), index=False)
                    if "daily_mean_long" in st.session_state:
                        st.session_state["daily_mean_long"].to_csv(os.path.join(td, "daily_mean_long.csv"), index=False)
                    if "daily_total_long" in st.session_state:
                        st.session_state["daily_total_long"].to_csv(os.path.join(td, "daily_total_long.csv"), index=False)

                    mlflow.log_artifacts(td, artifact_path="generated_data")
            # ---- /MLflow logging ----

        st.success("Precipitation generated, daily metrics computed, and baseline + per-board hourly precip shared with Forecasting agent.")
    ...
    st.info("✅ Baseline precipitation and spatial per-board hourly precip are now available to the 📈 Forecasting Agent (see sidebar).")
...
def forecasting_agent():
    ...
    precip_by_board_long = st.session_state.get(SESSION_KEYS["precip_by_board_long"])  # NEW
    ...
    with st.expander("See agent handoff", expanded=True):
        st.graphviz_chart('''
            digraph G {
                ...
                node [shape=box, style=rounded];
                "User" -> "Flood Mapping Agent";
                "Flood Mapping Agent" -> "Forecasting Agent" [label="precip time-series (baseline + per-board hourly)"];
                "Community Board Agent (beta)" -> "Forecasting Agent" [style=dashed, label="context (CB info)"];
                "Forecasting Agent" -> "User" [label="Explain + QA"];
            }
        ''')

    # --- Quick data handoff debug ---
    with st.expander("Data handoff status (debug)", expanded=False):
        precip_df_dbg = st.session_state.get(SESSION_KEYS["precip_df"])
        board_long_dbg = st.session_state.get(SESSION_KEYS["precip_by_board_long"])
        st.write({
            "precip_df rows": 0 if precip_df_dbg is None else len(precip_df_dbg),
            "per-board precip rows": 0 if board_long_dbg is None else len(board_long_dbg),
        })
        if isinstance(precip_df_dbg, pd.DataFrame) and not precip_df_dbg.empty:
            st.caption(f"Example ts_key in precip_df: { _ts_key(precip_df_dbg.iloc[0]['timestamp']) }")
...
We also cached a **per-board hourly** precipitation table so you can toggle a spatial precipitation layer for the same hour as the forecast.
            """)
    ...
    if model.startswith("Rule-based"):
        ...
        # ---- SAFE span wrapper (no-op if tracing unavailable) ----
        with mlflow_span("forecast_graph_diffusion"):
    ...
    # ---- MLflow logging: forecast run ----
    import tempfile, os  # avoid re-importing mlflow shadowing

    try:
        # Snapshot metrics at the first timestamp in fcst_long (or will recompute later when user selects)
        # We’ll log the overall spread too.
        overall_vals = pd.to_numeric(fcst_long["flood_index"], errors="coerce")
        mlflow_start_ts = str(fcst_long["timestamp"].iloc[0]) if len(fcst_long) else "n/a"

        with mlflow.start_run(run_name=f"forecast_{model.split()[0].lower()}_{mlflow_start_ts}"):
            mlflow.log_params({
                "model": model,
                "weights": weight_key,
                "timepoints": int(fcst_long["timestamp"].nunique()),
                "boards": int(fcst_long["unit_id"].nunique())
            })

            if overall_vals.notna().any():
                mlflow.log_metric("flood_index_min_all", float(overall_vals.min()))
                mlflow.log_metric("flood_index_max_all", float(overall_vals.max()))
                mlflow.log_metric("flood_index_mean_all", float(overall_vals.mean()))

            # Artifact: full forecast long table
            with tempfile.TemporaryDirectory() as td:
                out_path = os.path.join(td, "forecast_long.csv")
                fcst_long.to_csv(out_path, index=False)
                mlflow.log_artifact(out_path, artifact_path="forecast")
    except Exception:
        pass
    # ---- /MLflow logging ----

    # 3) Pick a time to visualize + QA  (MODIFIED: adds true spatial precip layer)
    ...
    # Toggle between Flood risk and Precipitation views
    layer_choice = st.radio(
        "Layer to display",
        ["Flood risk (index)", "Precipitation (mm/hr)"],
        horizontal=True,
        help="Switch between the model’s flood risk and the spatial precipitation field for the same hour."
    )
    ...
    # Series to render for the chosen layer
    if layer_choice.startswith("Flood"):
        # Flood risk snapshot (unchanged behavior)
        snap = fcst_long[fcst_long["timestamp"] == t_sel].set_index("unit_id")["flood_index"]
        s_current = snap.fillna(0.0)
        value_prop_name = "flood_index"
        tooltip_label = "Flood index"
        caption_explainer = "Flood risk (unitless index)"
    else:
        # --- Precipitation layer (board-level) ---
        s_current = None
        precip_by_board_long = st.session_state.get(SESSION_KEYS["precip_by_board_long"])
        precip_df = st.session_state.get(SESSION_KEYS["precip_df"])

        # Build a normalized key for the selected timestamp
        t_key = _ts_key(t_sel)

        if isinstance(precip_by_board_long, pd.DataFrame) and not precip_by_board_long.empty:
            if "ts_key" not in precip_by_board_long.columns:
                precip_by_board_long = precip_by_board_long.assign(ts_key=precip_by_board_long["timestamp"].apply(_ts_key))
                st.session_state[SESSION_KEYS["precip_by_board_long"]] = precip_by_board_long

            sub = precip_by_board_long.loc[precip_by_board_long["ts_key"] == t_key]
            if not sub.empty:
                s_current = sub.set_index("unit_id")["precip_mm_hr_board"]

        # Fallback: uniform (citywide) baseline value if spatial cache missing for this hour
        if s_current is None or (isinstance(s_current, pd.Series) and s_current.empty):
            if isinstance(precip_df, pd.DataFrame) and not precip_df.empty:
                if "ts_key" not in precip_df.columns:
                    precip_df = precip_df.assign(ts_key=precip_df["timestamp"].apply(_ts_key))
                    st.session_state[SESSION_KEYS["precip_df"]] = precip_df
                row = precip_df.loc[precip_df["ts_key"] == t_key]
                precip_val = float(row["precip_mm_hr"].iloc[0]) if not row.empty else 0.0
            else:
                precip_val = 0.0
            s_current = pd.Series({b["unit_id"]: precip_val for b in boards}, name="precip_mm_hr", dtype=float)

        value_prop_name = "precip_mm_hr"
        tooltip_label = "Precip (mm/hr)"
        caption_explainer = "Board-level precipitation (mm/hr) for this hour"

    br = _compute_breaks(s_current, k=7)

    # Build features for the chosen layer
    feats = []
    for b in boards:
        uid = b["unit_id"]
        v = float(s_current.get(uid, 0.0))
        feats.append({
            "type":"Feature",
            "properties":{
                "unit_id": uid,
    ...
    tooltip_html = f"<b>Board #:</b> {{cb}}<br/><b>{tooltip_label}:</b> {{{value_prop_name}:.2f}}"
    deck = pdk.Deck(
        layers=layers_fc,
        initial_view_state=pdk.ViewState(latitude=lat_c, longitude=lon_c, zoom=zoom_c),
    ...
    st.caption(f"Layer: **{layer_choice}** — {caption_explainer} at {t_sel_str}")

    # Legend (for the currently displayed layer)
    with st.expander("Color Legend (current layer)", expanded=False):
        legend_rows = []
        rngs = [float(s_current.min())] + br + [float(s_current.max())]
        for i in range(len(BLUES)):
            legend_rows.append({
                "Range": f"{rngs[i]:.2f} – {rngs[i+1]:.2f}",
                "Color (RGB)": f"rgb({BLUES[i][0]}, {BLUES[i][1]}, {BLUES[i][2]})"
    ...
    # Preserve existing downloads + captions exactly
    st.download_button(
    ...
    # Preserve assistant context based on the FLOOD layer (so fields/labels remain consistent)
    snap_flood = fcst_long[fcst_long["timestamp"] == t_sel].set_index("unit_id")["flood_index"].fillna(0.0)
    context_forecast = {
        "type": "forecast_snapshot",
        "timestamp": t_sel_str,
        "method": method_desc,
        "static_weights": weight_key,
        "by_board": [{"cb": get_board_display_id(b), "flood_index": float(snap_flood.get(b['unit_id'], 0.0))} for b in boards],
        "stats": {
            "min": float(snap_flood.min()),
            "max": float(snap_flood.max()),
            "mean": float(snap_flood.mean()),
            "median": float(snap_flood.median())
        }
    }
    render_map_assistant(
    ...
    # 4) Aggregate forecast over a selected time range  (UNCHANGED)
    st.divider()
