...
def flooding_mapping_agent():
    ...
    try:
        attrs_sample = pd.DataFrame([b.get("attrs", {}) | {"unit_id": b["unit_id"], "name": b.get("name", "")} for b in boards]).head(10)
        st.write("**Community Board attributes (sample):**")
        st.dataframe(attrs_sample, hide_index=True, use_container_width=True)
    except Exception:
        pass
